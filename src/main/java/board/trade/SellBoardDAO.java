...
public class SellBoardDAO extends DBConnPool{
	...
	public List<SellBoardDTO> selectListPage(Map<String, Object> map){
		...
		//카테고리 분류 보기
			...
			if (map.get("searchWord") != null) {
				query += " AND sell_category=0 AND sell_category=1 AND sell_category=2 AND sell_category=3 "
						+ "AND sell_category=4 AND sell_category=5 AND sell_category=6 "
						+ "OR " + searchField + " LIKE '%" +searchWord + "%' ";
