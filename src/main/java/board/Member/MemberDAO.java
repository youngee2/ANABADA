...
//		String query = "SELECT * FROM MEMBERTB WHERE user_id=? AND user_passwd=?"; // 쿼리문 템플릿
//
//		try {
//			// 쿼리 실행
...
//				if (rs.getString(1).equals(user_passwd))
//					return 1; // 로그인 성공
//				else
//					return 0; // 불일치
//
...
//	}
	...
	public int UpdateInfo(MemberDTO dto) {
		int result = 0;
		try {
			String query = "UPDATE MEMBERTB SET name =?, NICKNAME = ?, USER_PASSWD =?,"
					+ " EMAIL = ? , PHONE_NUM =?, user_picture ='?' WHERE user_id ='?'";
